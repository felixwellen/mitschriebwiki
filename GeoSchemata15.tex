...
\item Der von $\pi: R \longrightarrow \slant{R}{N_R}$ induzierte Schemamorphismus ist ein Homoömorphismus. Er macht $X_{\textrm{red}}$ zu einem abgeschlossenen Unterschema von $X$.
...
Ein Morphismus $f: X \la Y$ vom Schemata heißt \textit{endlich}, falls es eine offene affine Überdeckung $\{U_i = \spec A_i\}_{i \in I}$ von $Y$ gibt, sodass $f^{-1}(U_i) = \spec B_i$ für jedes $i \in I$ affin und $B_i$ endlich erzeugter $A_i$-Modul ist.
\end{defin}
...
\begin{pr}
Es werden nur jeweils die Implikation \enquote{$\Rightarrow$} gezeigt - die Rückrichtung lässt sich beispielsweise in Hartshorne II.4.3 bzw. II.4.7 nachlesen.
\begin{compactenum}
...
Weiter sind die globalen Schnitten
...
\item Sei $\Omega= \Omega_{k(X)/k}$ der $k(X)$-Vektorraum der Kählerdifferentiale von $k(X)/k$. Die Elemente von $\Omega$ heißen rationale Differentiale auf $X$. Ohne Einschränkung gelte $X=V(f)$ mit einem irreduziblen Polynom $f \in k[X,Y]$. Dann ist $k(X) = \textrm{Quot} \slant{k[X,Y]}{(f)}$. Damit wird $\Omega$ erzeugt von den Elementen $\textrm{d}g$ für $g \in k(X)$, wobei $\textrm{d}$ die universelle Derivation bezeichne. Da $\textrm{d}(X^2) = 2X \textrm{d}X$und induktiv $\textrm{d}(X^n) = n X^{n-1} \textrm{d}X$, genügen die linearen Terme. $\textrm{d}f=0$ ergibt also eine lineare Gleichung zwischen $\textrm{d}X$ und $\textrm{d}Y$ und wir erhalten $\dim_{k(X)} \Omega =1$. Wir wollen uns daraus nun eine $\mathcal{O}_X$-Modulgarbe basteln. Für $ \omega \in \Omega_{k(X)/k}$ sei
...
\begin{compactenum}
\item Es bildet $\Hom_{\C}(A,B)$ eine abelsche Gruppe bezüglich der Addition \enquote{$+$} für alle $A,B \in \mathrm{Ob}(\C)$.
\item Für Homomorphismen gelten die Distributivgesetze bezüglich \enquote{$+$} und \enquote{$\circ$}, das heißt es gilt
...
also gilt, da $H^{0}\left(X, \slant{\G_0}{\F}\right) \la H^{1}(X, \F)$ Epimorphismus ist
\setlength{\abovedisplayskip}{5.5pt}
\setlength{\belowdisplayskip}{5.5pt}
\begin{alignat*}{5}
...
Dann ist $\Phi$ nichtleer und durch \enquote{$\leqslant$} geordnet, enthält nach Zorns Lemma also ein maximales Element $(A_{\textrm{max}}, \phi_{\textrm{max}})$. Wäre $A_{\textrm{max}} \neq A$, so wähle $\overline{a} \in A \setminus \overline{A}$ und verfahre wir oben und führe diesen Fall zum Widerspruch. Damit folgt die Behauptung. $\hfill \Box$
\end{ex}
...
Sei $\lokring$ ein Schema, $\F$ eine Garbe abelscher Gruppen auf $X$ und 
